...
import { editTag } from "../../../../store/Actions/adminAction";
import { toastr } from "react-redux-toastr";
import LoaderComponent from "components/Common/Loader/LoaderComponent";

const EditTagModal = ({ openModal, hideModal, callBack, editTag, tagData }) => {
  ...
  const onFormSubmit = async () => {
    try {
      if (!tagData) {
        toastr.error("Error", "No Tag detail found");
        return;
      }
      setLoader(true);
      let result = await editTag(title, description, tagData.id);
      setLoader(false);
    ...
    <>
      <Modal isOpen={openModal} toggle={hideModalHandler}>
        <div className="modal-header">
          <h5 className="modal-title mt-0">Edit Tag Details</h5>
          <button
